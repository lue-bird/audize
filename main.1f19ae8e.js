...
var $pd_andy$elm_web_audio$WebAudio$Program$document = function (_v0) {
	var init = _v0.init;
...
var $author$project$Main$Adjective = {$: 'Adjective'};
var $author$project$Main$Noun = {$: 'Noun'};
var $author$project$Main$Other = {$: 'Other'};
var $author$project$Main$Person = {$: 'Person'};
var $author$project$Main$UseGeneratedSong = function (a) {
	return {$: 'UseGeneratedSong', a: a};
};
var $author$project$Main$Verb = {$: 'Verb'};
...
var $author$project$Main$generateSong = function () {
	var randomWord = A2(
		...
		A2(
			$elm$random$Random$uniform,
			$author$project$Main$Verb,
			_List_fromArray(
				[$author$project$Main$Person, $author$project$Main$Noun, $author$project$Main$Adjective, $author$project$Main$Other])));
	return A2(
		$elm$random$Random$generate,
		$author$project$Main$UseGeneratedSong,
		A2(
			$elm$random$Random$andThen,
			function (songSeconds) {
				return A3(
					...
					F2(
						function (tones, name) {
							return {duration: songSeconds * 1000, name: name, tones: tones};
						}),
					A2(
						...
						A5(
							...
							F4(
								function (frequency, duration, begin, volume) {
									return {
										begin: begin,
										duration: A2($elm$core$Basics$min, duration, (songSeconds * 1000) - begin),
										frequency: frequency,
										volume: volume
									};
								}),
							A2($elm$random$Random$float, 80, 800),
							A2($elm$random$Random$int, 100, 5000),
							A2($elm$random$Random$int, 0, songSeconds * 1000),
							A2($elm$random$Random$float, 0.1, 1.2))),
					A2(
						$elm$random$Random$map,
						$elm$core$String$join(' '),
						A2(
							$elm$random$Random$list,
							2 + $elm$core$Basics$round(
								$elm$core$Basics$sqrt(songSeconds) / 4),
							randomWord)));
			},
			A2($elm$random$Random$int, 10, 60 * 3)));
}();
var $author$project$Main$Stopped = {$: 'Stopped'};
var $author$project$Main$initialModel = {
	playing: $author$project$Main$Stopped,
	song: {duration: 0, name: '', tones: _List_Nil},
	time: 0
};
...
var $author$project$Main$play = function (model) {
	...
	if (_v0.$ === 'Stopped') {
		...
		return A2(
			$elm$core$List$filterMap,
			function (_v1) {
				var frequency = _v1.frequency;
				var duration = _v1.duration;
				var begin = _v1.begin;
				var volume = _v1.volume;
				var playedFor = model.time - begin;
				return ((playedFor >= 0) && (_Utils_cmp(playedFor, duration) < 0)) ? $elm$core$Maybe$Just(
					A2(
						$pd_andy$elm_web_audio$WebAudio$oscillator,
						_List_fromArray(
							[
								$pd_andy$elm_web_audio$WebAudio$Property$frequency(frequency)
							]),
						_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										...
										((1.8 * volume) * A2($elm$core$Basics$pow, playedFor / duration, 2.5)) * A2($elm$core$Basics$pow, (duration - playedFor) / duration, 2.5))
									]),
								_List_fromArray(
									[$pd_andy$elm_web_audio$WebAudio$audioDestination]))
							]))) : $elm$core$Maybe$Nothing;
			},
			model.song.tones);
	}
...
var $author$project$Main$ToTime = function (a) {
	return {$: 'ToTime', a: a};
};
...
var $author$project$Main$subscriptions = function (model) {
	return (_Utils_eq(model.playing, $author$project$Main$Playing) && (_Utils_cmp(model.time, model.song.duration) < 0)) ? A2(
		$elm$time$Time$every,
		10,
		function (_v0) {
			return $author$project$Main$ToTime(model.time + 10);
		}) : $elm$core$Platform$Sub$none;
};
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'GenerateNewSong':
				return _Utils_Tuple2(model, $author$project$Main$generateSong);
			case 'UseGeneratedSong':
				var song = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{playing: $author$project$Main$Playing, song: song, time: 0}),
					$elm$core$Platform$Cmd$none);
			case 'Stop':
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'Play':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{playing: $author$project$Main$Playing}),
					$elm$core$Platform$Cmd$none);
			default:
				var newTime = msg.a;
				return _Utils_Tuple2(
...
var $author$project$Main$GenerateNewSong = {$: 'GenerateNewSong'};
...
var $author$project$Main$formatMilliSeconds = function (ms) {
	var seconds = A2($elm$core$Basics$modBy, 60, (ms / 1000) | 0);
	return $elm$core$String$fromInt((((ms / 1000) | 0) / 60) | 0) + (':' + ((seconds < 10) ? ('0' + $elm$core$String$fromInt(seconds)) : $elm$core$String$fromInt(seconds)));
};
var $mdgriffith$elm_ui$Element$rgb = F3(
	function (r, g, b) {
		return A4($mdgriffith$elm_ui$Internal$Model$Rgba, r, g, b, 1);
	});
var $author$project$Main$inputColor = A3($mdgriffith$elm_ui$Element$rgb, 1, 0.6, 0);
...
var $author$project$Main$input = function (ui_) {
	return A2(
		...
		_List_fromArray(
			[
				ui_,
				A2($author$project$Main$circle, 5, $author$project$Main$inputColor)
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Font$typeface('Noto Sans')
					])),
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$text('audize')),
				A2(
				...
				{
					label: $author$project$Main$input(
						A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
								[
					...
					onPress: $elm$core$Maybe$Just($author$project$Main$GenerateNewSong)
				}),
				A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
					...
					[
						...
						function () {
							var _v0 = function () {
								...
								if (_v1.$ === 'Playing') {
									return {onPress: $author$project$Main$Stop, text: '⏸'};
								} else {
									return {onPress: $author$project$Main$Play, text: '⏵'};
								}
							}();
							var onPress = _v0.onPress;
							var text = _v0.text;
								...
								A2(
									...
									_List_fromArray(
										[
											...
											$mdgriffith$elm_ui$Element$centerY,
											$mdgriffith$elm_ui$Element$Background$color($author$project$Main$inputColor),
							...
							]),
						{
							label: A2(
								...
								_List_Nil,
								$mdgriffith$elm_ui$Element$text(
									$author$project$Main$formatMilliSeconds(model.time) + (' / ' + $author$project$Main$formatMilliSeconds(model.song.duration)))),
							max: model.song.duration,
							min: 0,
							onChange: function (newTime) {
								return $author$project$Main$ToTime(
									$elm$core$Basics$round(newTime));
							},
						...
						$mdgriffith$elm_ui$Element$text(model.song.name))
					]))
			]));
};
			...
			[
				...
				$mdgriffith$elm_ui$Element$layoutWith,
				{
					options: _List_fromArray(
						[
							$mdgriffith$elm_ui$Element$noHover,
...
var $author$project$Main$main = $pd_andy$elm_web_audio$WebAudio$Program$document(
	{
		audio: $author$project$Main$play,
		audioPort: $author$project$Main$updateAudio,
		init: function (_v0) {
			return _Utils_Tuple2($author$project$Main$initialModel, $author$project$Main$generateSong);
		},
...
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Main.Song":{"args":[],"type":"{ tones : List.List Main.Tone, duration : Basics.Int, name : String.String }"},"Main.Tone":{"args":[],"type":"{ frequency : Basics.Float, duration : Basics.Int, begin : Basics.Int, volume : Basics.Float }"}},"unions":{"Main.Msg":{"args":[],"tags":{"GenerateNewSong":[],"UseGeneratedSong":["Main.Song"],"Stop":[],"Play":[],"ToTime":["Basics.Int"]}},"Basics.Float":{"args":[],"tags":{"Float":[]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"List.List":{"args":["a"],"tags":{}},"String.String":{"args":[],"tags":{"String":[]}}}}})}});
...
var VirtualAudioGraph = /*#__PURE__*/function () {
  // Constructor ===============================================================
  //
  function VirtualAudioGraph() {
    ...
    // on construction.
    this.$context = context; // A reference to the real graph of audio nodes

    this.$nodes = {}; // We keep track of the prebious graph so we can perform a diff and work out
    // what has changed between updates.

    this.vPrev = {}; // In most modern browsers an Audio Context starts in a suspended state and
  ...
  _createClass(VirtualAudioGraph, [{
    ...
    value: function update() {
      ...
      var vGraph = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
      ...
      var vCurr = VirtualAudioGraph.prepare(vGraph); // A diff tracks everything that has been removed, created,
      // and updated between updates. We perform this step so we
      // only touch the audio nodes that need to be changed in some
      // way.

      var diff = VirtualAudioGraph.diff(this.vPrev, vCurr); // Remove nodes and properties from the graph.

      diff.removed.forEach(function (patch) {
        switch (patch.type) {
          case 'node':
            _this._destroyNode(patch.key);

            break;

          case 'property':
            _this._removeProperty(patch.key, patch.data);

            break;

          case 'connection':
            _this._disconnect(patch.key, patch.data);
      ...
      diff.created.forEach(function (patch) {
        switch (patch.type) {
          case 'node':
            _this._createNode(patch.key, patch.data);

            break;

          case 'property':
            _this._setProperty(patch.key, patch.data);

            break;

          case 'connection':
            defer(function () {
              return _this._connect(patch.key, patch.data);
            });
            break;
        }
      ...
      diff.updated.forEach(function (patch) {
        switch (patch.type) {
          case 'node':
            _this._destroyNode(patch.key);
    ...
    key: "_createNode",
    value: function _createNode(key, _ref) {
      var _this2 = this;
      ...
      switch (type) {
        ...
        case 'DelayNode':
          var maxDelayTime = properties.find(function (_ref2) {
            var label = _ref2.label;
            return label === 'maxDelayTime';
          });
        ...
        case 'IIRFilterNode':
          var feedforward = properties.find(function (_ref3) {
            var label = _ref3.label;
            return label === 'feedforward';
          });
          var feedback = properties.find(function (_ref4) {
            var label = _ref4.label;
            return label === 'feedback';
          });
          $node = this.$context.createIIRFilter(feedforward && feedforward.value || [0], feedback && feedback.value || [1]);
          break;

        case 'MediaElementAudioSourceNode':
          var mediaElement = properties.find(function (_ref5) {
            var label = _ref5.label;
            return label === 'mediaElement';
          });
          $node = this.$context.createMediaElementSource(document.querySelector(mediaElement.value));
          break;
        ...
        default:
          console.warn("Invalide node type of: ".concat(type, ". Defaulting to GainNode to avoid crashing the AudioContext."));
          $node = this.$context.createGain();
      }
      ...
      properties.forEach(function (prop) {
        return _this2._setProperty(key, prop);
    ...
    key: "_destroyNode",
    value: function _destroyNode(key) {
      var $node = this.$nodes[key]; // Certain nodes like oscillators can be stopped. It probably doesn't make
      // much of a difference calling this method, but we do just in case!

      if ($node.stop) $node.stop(); // Calling disconnect with no arguments will disconnect this node from
      // everything.

      $node.disconnect(); // Finally remove the node from the graph and let the GC do its job.
    ...
    key: "_setProperty",
    value: function _setProperty(key, _ref6) {
      var type = _ref6.type,
          label = _ref6.label,
          value = _ref6.value;
    ...
    key: "_removeProperty",
    value: function _removeProperty(key, _ref7) {
      var type = _ref7.type,
          label = _ref7.label,
          value = _ref7.value;
      var $node = this.$nodes[key];
    ...
    key: "_connect",
    value: function _connect(a, _ref8) {
      var _ref9 = _slicedToArray(_ref8, 2),
          b = _ref9[0],
          _ref9$ = _ref9[1],
          param = _ref9$ === void 0 ? null : _ref9$;
    ...
    key: "_disconnect",
    value: function _disconnect(a, _ref10) {
      var _ref11 = _slicedToArray(_ref10, 2),
          b = _ref11[0],
          _ref11$ = _ref11[1],
          param = _ref11$ === void 0 ? null : _ref11$;
    ...
    function prepare() {
      ...
      var key = function key(graph) {
        ...
        graph.forEach(function (node, i) {
          ...
          if (node.type === 'RefNode') return; // Assign the node a key if it didn't already have one.
          // This is how we track changes to the graph in a slightly
          // more organised way

          if (!node.key) node.key = "".concat(base, "_").concat(i); // Recursively assign keys to this nodes connections.
      ...
      var flatten = function flatten(graph) {
        var nodes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var depth = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
        graph.forEach(function (node, i) {
          // Don't push RefNodes to the flat graph.
          if (node.type !== 'RefNode') nodes[node.key] = node;
          if (node.connections) flatten(node.connections, nodes, depth + 1); // If we're deeper than the root of the graph, replace
    ...
    key: "diff",
    value: function diff(oldNodes, newNodes) {
      var patches = {
        created: [],
        updated: [],
        removed: []
      ...
      var _loop = function _loop() {
        var newNode = _Object$values[_i2];
        var oldNode = oldNodes[newNode.key]; // A node with newNode.key does not exist in the old graph, so this must
        ...
        if (!oldNode) {
          ...
          newNode.connections.forEach(function (connection) {
            patches.created.push({
              type: 'connection',
              key: newNode.key,
          ...
          // Checking properties...
          for (var j = 0; j < Math.max(oldNode.properties.length, newNode.properties.length); j++) {
            var oldProp = oldNode.properties[j];
            var newProp = newNode.properties[j]; //
          ...
          for (var _j = 0; _j < Math.max(oldNode.connections.length, newNode.connections.length); _j++) {
            var oldConnection = oldNode.connections[_j];
            var newConnection = newNode.connections[_j]; //
      ...
      for (var _i2 = 0, _Object$values = Object.values(newNodes); _i2 < _Object$values.length; _i2++) {
        _loop();
      }

      for (var _i3 = 0, _Object$values2 = Object.values(oldNodes); _i3 < _Object$values2.length; _i3++) {
        var oldNode = _Object$values2[_i3];
        patches.removed.push({
          type: 'node',
          key: oldNode.key,
          data: oldNode
...
var _virtualAudio = _interopRequireDefault(require("./virtual-audio"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

/* global AudioContext */
var context = new AudioContext();
var audio = new _virtualAudio.default(context); // Chrome autplay policy demans some user interaction
